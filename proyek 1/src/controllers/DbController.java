...
public class DbController extends MyConfig {
    ...
    public static void getDatabase() {
        ...
        try {
            ...
            query = "UPDATE eva, harga, jumlah FROM eva_produk";
    ...
    public static Produk getProdukByNama(String nama) {
        Produk produk = null;
        connection();
        query = "SELECT * FROM eva_produk WHERE nama=?";
        try {
            preparedStatement = connection.prepareStatement(query);
    ...
    public static boolean insertData(String nama, long harga, int jumlah) {
        connection();
        query = "INSERT INTO eva_produk (nama, harga, jumlah) VALUES (?, ?, ?)";
        try {
            preparedStatement = connection.prepareStatement(query);
            preparedStatement.setString(1, nama);
    ...
    public static void updateNama(int id, String nama) {
        connection();
        query = "UPDATE eva_produk SET nama=? WHERE id=?";
        try {
            preparedStatement = connection.prepareStatement(query);
            preparedStatement.setString(1, nama);
    ...
    public static void updateHarga(int id, long harga) {
        connection();
        query = "UPDATE eva_produk SET harga=? WHERE id=?";
        try {
            preparedStatement = connection.prepareStatement(query);
    ...
    public static void updateStok(int id, int jumlah) {
        connection();
        query = "UPDATE eva_produk SET jumlah=? WHERE id=?";
        try {
            preparedStatement = connection.prepareStatement(query);
